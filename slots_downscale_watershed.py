...
parser.add_argument('-pd', help='Directory to locate preprocessed files',
                    default='/work/scratch-nopw/wkjones/slots_preprocess_florida/',
                    type=str)
parser.add_argument('-sd', help='Directory to save processed files',
                    default='/work/scratch-nopw/wkjones/slots_process_florida/',
                    type=str)
...
inner = slots.watershed(edges, flow, markers, mask=mask,
                        structure=ndi.generate_binary_structure(3,2)
                        ).astype(bool)

inner_labels = slots.flow_label(inner, flow)
...
outer = slots.watershed(edges, flow, new_markers, mask=mask,
                        structure=ndi.generate_binary_structure(3,2)
                        ).astype(bool)

outer_labels = slots.flow_label(outer, flow)

del edges

ds = xr.Dataset({'inner':xr.DataArray(inner_labels.astype(bool), dims=out_dims, coords=out_coords),
                 'outer':xr.DataArray(outer_labels.astype(bool), dims=out_dims, coords=out_coords)})

save_name = 'slots_process_S'\
